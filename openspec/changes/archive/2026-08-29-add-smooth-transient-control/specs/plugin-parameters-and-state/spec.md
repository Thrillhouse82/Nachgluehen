...
The plugin SHALL expose Freeze as a boolean parameter defaulting to disabled, Drift as a continuous UI percentage parameter in the range 0-100% defaulting to 20%, Smooth as a continuous UI percentage parameter in the range 0-100% defaulting to 50%, Dry/Wet as a continuous UI percentage parameter in the range 0-100% defaulting to 50%, and Output Gain as a continuous dB parameter with a mute endpoint, a maximum of +12 dB, and a default of 0 dB. Drift and Smooth values SHALL map linearly to normalized processing ranges 0.0-1.0. Output Gain SHALL use a monotonic invertible asymmetric normalized mapping with mute at 0.0, 0 dB at 0.5, and +12 dB at 1.0.

#### Scenario: Defaults are available
- **WHEN** a new plugin instance is created
- **THEN** Freeze is off, Drift is 20%, Smooth is 50%, Dry/Wet is 50%, and Output Gain is 0 dB

#### Scenario: Values stay in range
- **WHEN** a host or UI supplies parameter values
- **THEN** Drift, Smooth, and Dry/Wet are constrained to 0-100%, Output Gain is constrained between its mute endpoint and +12 dB, and Freeze resolves to a boolean state
...
#### Scenario: Smooth mapping is normalized
- **WHEN** the UI or host supplies Smooth values of 0%, 50%, and 100%
- **THEN** the DSP receives normalized values of 0.0, 0.5, and 1.0 respectively
...
- **THEN** the final output is respectively muted or amplified by the corresponding bounded gain without changing the Freeze, Drift, Smooth, or Dry/Wet settings

### Requirement: Host automation
The plugin SHALL expose Freeze, Drift, Smooth, Dry/Wet, and Output Gain as host-automatable parameters whose changes affect processing predictably. Smooth automation SHALL be smoothed so that changes to the wet texture do not introduce clicks, abrupt gain jumps, or uncontrolled DSP artifacts. Output Gain automation and restored state SHALL use the same centred asymmetric mapping as the editor.
...
- **WHEN** the host automates Drift, Smooth, Dry/Wet, or Output Gain
- **THEN** the processor applies the new value without invalid output or an abrupt uncontrolled parameter jump

#### Scenario: Smooth automation reaches the DSP continuously
- **WHEN** Smooth is automated across its full 0-100% UI range while Freeze is enabled
- **THEN** the Living-Freeze texture approaches the corresponding transient-character target continuously
...
The plugin SHALL serialize and restore all visible parameter values, including Freeze, Drift, Smooth, Dry/Wet, and Output Gain, in the plugin state.
...
- **THEN** Freeze, Drift, Smooth, Dry/Wet, and Output Gain match the saved values, including the Output Gain dB value under its centred mapping
