...
- **WHEN** the UI or host inspects the published continuous parameter mappings
- **THEN** no Smooth parameter or Smooth normalized DSP mapping is present
...
- **WHEN** the host inspects available automation targets while Freeze is enabled
- **THEN** Smooth is not an automatable target and no Smooth value reaches the Living-Freeze engine
